<template>
  <div>
    ...
    <Modal :closable="false" :mask-closable="false" v-model="modalChoosePlane">
      <h1 slot="header" class="h1_modal">选择战斗机</h1>
      <div style="text-align:center">
        <img class="img_src" id="img_player1" src="../assets/player1.svg" />
        <img class="img_src" id="img_player2" src="../assets/player3.svg" />
      </div>
      <div slot="footer" style="text-align:center">
        <Button type="primary" size="large" @click="choosePlane(1)">选择1</Button>
        <Button type="primary" size="large" @click="choosePlane(2)">选择2</Button>
      </div>
    </Modal>
    ...
    <Modal :closable="false" :mask-closable="false" v-model="modalHelp">
      ...
      <div>
        <p class="p_modal">1. 破坏敌方单位将获得积分；</p>
        <p class="p_modal">2. 当玩家血条为0时游戏结束；</p>
    ...
    <Modal :closable="false" :mask-closable="false" v-model="modalResult">
      ...
      <div>
        <p class="p_modal">你获得了 {{binderScore}} 分，</p>
...
export default {
  ...
  mounted () {
    if (typeof localStorage.solo !== 'undefined') {
  ...
  methods: {
    onRecover: function (n) {
      ...
      if (n) {
        var lastInfo = JSON.parse(localStorage.solo)
        // console.log(lastInfo)
        localStorage.removeItem('solo')
        if (this.gameState === mGame.GameState.CREATE) {
          this.prepareEnv()
          this.prepareImg()
          this.choosePlane(lastInfo.playerId, false)
    ...
    recoverSrc: function (data) {
      this.binderScore = data.binderScore
      this.binderHp = data.binderHp
      this.binderLaser = data.binderLaser
      this.binderNuclear = data.binderNuclear
      this.binderShield = data.binderShield
      this.playerId = data.playerId
      this.isShotting = data.isShotting
      this.alertPause = data.alertPause
      this.animationIdLoop = data.animationIdLoop
      this.timeoutIdEnemy = data.timeoutIdEnemy
      this.timeoutIdBlock = data.timeoutIdBlock
      this.timeoutIdBoss = data.timeoutIdBoss
      this.timeoutIdLaser = data.timeoutIdLaser
      this.timeoutIdNuclear = data.timeoutIdNuclear
      this.timeoutIdShield = data.timeoutIdShield
      this.timeoutIdStop = data.timeoutIdStop
      data.enemyBullets.forEach(element => {
        var eb = new mBullet.Bullet(
          this.canvas,
          element.x,
          element.y,
          element.offsetX,
      ...
      data.enemies.forEach(element => {
        var eb = new mEnemy.Enemy(
          this.canvas,
          element.x,
          element.y,
          element.offsetX,
          element.offsetY,
          this.imgsEnemy[element.order],
          this.imgExplosion,
          element.order
        )
        eb.setState(element.hp, element.alive, element.show)
        this.enemies.push(eb)
      })
      data.blocks.forEach(element => {
        var eb = new mBlock.Block(
          this.canvas,
          element.x,
          element.y,
          element.offsetX,
      ...
      this.player.setState(data.player.hp, data.player.alive, data.player.laser, data.player.shield)
      if (data.nuclear !== null) {
        this.nuclear = new mNuclear.Nuclear(
          this.canvas,
          data.nuclear.x,
          data.nuclear.y,
          data.nuclear.offsetY,
          this.imgNuclear,
          this.imgNuclearExplosion
        )
        this.nuclear.setState(data.nuclear.hp, data.nuclear.alive, data.nuclear.show)
      }
    },
    choosePlane: function (n, help = true) {
      if (n === 1) {
        this.playerId = 1
      } else {
        this.playerId = 3
      }
      this.imgPlayer.src = require('../assets/player' + this.playerId + '.svg')
      this.imgShield.src = require('../assets/shield' + this.playerId + '.svg')
      // console.log(this.imgPlayer.src)
      this.modalChoosePlane = false
      this.modalHelp = help
    },
    /** Lifecycle */
    onPrepare: function () {
    ...
    onSave: function (save) {
      ...
      delete saveData.ctx
      delete saveData.imgPlayer
      delete saveData.imgExplosion
      delete saveData.imgNuclear
      delete saveData.imgNuclearExplosion
      delete saveData.imgLaser
      delete saveData.imgShield
      delete saveData.imgsEnemy
      delete saveData.imgsBoss
      delete saveData.imgsBlock
      delete saveData.player.ctx
      delete saveData.player.imgAlive
      delete saveData.player.imgExplosion
      delete saveData.player.imgLaser
      delete saveData.player.imgShield
      delete saveData.player.ctx
      saveData.enemies.forEach(element => {
        delete element.ctx
        delete element.imgAlive
        delete element.imgExplosion
      })
      saveData.blocks.forEach(element => {
        delete element.ctx
        delete element.imgAlive
        delete element.imgExplosion
      })
      saveData.bosses.forEach(element => {
        delete element.ctx
        delete element.imgAlive
        delete element.imgExplosion
      })
      saveData.enemyBullets.forEach(element => {
        delete element.ctx
      })
      saveData.playerBullets.forEach(element => {
        delete element.ctx
      })
      delete saveData.colController
      delete saveData.dataController
      if (saveData.nuclear !== null) {
        delete saveData.nuclear.ctx
        delete saveData.nuclear.imgAlive
        delete saveData.nuclear.imgExplosion
      }
      // console.log(JSON.stringify(saveData))
      localStorage.setItem('solo', JSON.stringify(saveData))
      this.$router.push({
        path: `/`
      })
    }
